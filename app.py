...
import os
import uuid
from streamlit_gsheets import GSheetsConnection  # Potrebná knižnica

# --- NASTAVENIA STRÁNKY ---
st.set_page_config(page_title="Minúty 2026", layout="centered")

# --- KONFIGURÁCIA GOOGLE SHEETS ---
# V Streamlit Cloud pridaj do Secrets:
# [connections.gsheets]
# spreadsheet = "URL_TVOJHO_SHEETU"
conn = st.connection("gsheets", type=GSheetsConnection)

# --- SÚBORY (Lokálna záloha) ---
NAMES_FILE = "Zoznam_mien.txt"
DATA_FILE = "data.csv"

# Inicializácia lokálnych súborov
...
def load_data():
    try:
        # Primárne načítanie z Google Sheets
        df = conn.read(ttl=0) # ttl=0 zabezpečí vždy čerstvé dáta
        df['Date'] = pd.to_datetime(df['Date']).dt.date
        df['Hodnota'] = df['Hodnota'].astype(str).str.zfill(3)
        return df
    except:
        # Fallback na lokálne CSV ak Sheets zlyhajú
        if not os.path.exists(DATA_FILE):
...
def save_data(df):
    df['Hodnota'] = df['Hodnota'].astype(str).str.zfill(3)
    # Uloženie do lokálneho CSV (záloha)
...
# (Zvyšok kódu ostáva úplne identický ako vo vašom zadaní...)
